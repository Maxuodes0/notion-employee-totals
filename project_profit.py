...
# ثم يكتب في قاعدة "ربحية المشاريع" (Upsert) مع Relation للمشروع

import os, re, time, requests
from collections import defaultdict

# ========= ضع معرّف قاعدة Projects (ينفع 32 خانة/بشرطات/حتى رابط) =========
...
# ✅ الحل A: نفس نسخة API حق main.py
HDRS = {
...
def hyphenate(nid: str) -> str:
    ...
    if re.fullmatch(r"[0-9a-fA-F]{32}", nid):
        return f"{nid[0:8]}-{nid[8:12]}-{nid[12:16]}-{nid[16:20]}-{nid[20:32]}".lower()
    return nid.lower()

# لاحقًا بنبدّل PROJECTS_DB_ID بالـ ID الرسمي من Notion
PROJECTS_DB_ID = hyphenate(extract_uuid32(RAW_PROJECTS_DB_ID))
...
def _query_once(db_id, page_size=100, limit=None, filter_payload=None):
...
def query_database_pages_robust(db_id, page_size=100, limit=None, filter_payload=None):
    """جرّب بالشكل المرسل، ولو 400 invalid_request_url جرّب نفس الـID لكن بدون شرطات."""
    try:
        return _query_once(db_id, page_size, limit, filter_payload)
    except requests.HTTPError as e:
        if e.response is not None and e.response.status_code == 400:
            raw = re.sub(r"-", "", db_id)
            if raw != db_id:
                return _query_once(raw, page_size, limit, filter_payload)
        raise
...
def sum_numeric_column(db_id, col_name):
    ...
    total = 0.0
    for row in query_database_pages_robust(db_id, page_size=100):
...
def get_existing_rows_map(db_id: str):
    pages = query_database_pages_robust(db_id, page_size=100)
    m = {}
    for p in pages:
...
def aggregate_and_write():
    # 1) ثبّت الـID الرسمي من Notion (بشرطات)، واطبع النسختين لأغراض التشخيص
    info = retrieve_database(PROJECTS_DB_ID)
    proj_title = (info.get("title") or [{}])[0].get("plain_text", "(No title)")
    proj_id_canon = info.get("id")  # الشكل الموثّق من Notion
    proj_id_raw   = proj_id_canon.replace("-", "")
    print(f"✅ Projects DB: {proj_title}")
    print(f"🔎 Using Projects ID: {proj_id_canon} (raw={proj_id_raw}) | Notion-Version={HDRS['Notion-Version']}")

    # 2) اقرأ المشاريع (بـ robust query)
    projects = query_database_pages_robust(proj_id_canon, page_size=100, limit=MAX_PROJECTS)
    print(f"📦 عدد المشاريع: {len(projects)}")

    # 3) جهّز قاعدة المخرجات
    out_db_id = ensure_output_db(proj_id_canon)
    ensure_output_columns(out_db_id, proj_id_canon)
    ...
    try:
        url = retrieve_database(out_db_id).get("url")
        print(f"📄 افتح القاعدة مباشرة: {url}")
    except Exception:
        pass
    print("🎯 اكتمل التحديث.")
