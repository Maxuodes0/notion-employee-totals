...
PROFIT_DB_ID_ENV       = os.getenv("PROFIT_DB_ID")          # اكتب هنا مباشرة لو موجود
PROFIT_PARENT_PAGE_ENV = os.getenv("PROFIT_PARENT_PAGE_ID") # أو أنشئ تحت هذه الصفحة

# ضريبة القيمة المضافة لو اضطرينا نحول من شامل إلى غير شامل
...
def _req(m,u,**k):
    for i in range(6):
        try:
            r = requests.request(m,u,headers=HDRS,timeout=DEFAULT_TIMEOUT,**k)
...
def GET(u,p=None):  return _req("GET",u,params=p)
def POST(u,j=None): return _req("POST",u,json=j or {})
def PATCH(u,j=None):return _req("PATCH",u,json=j or {})
...
def retrieve_db(db_id):   return GET(f"https://api.notion.com/v1/databases/{db_id}").json()
def query_db(db_id, page_size=100):
...
def find_child_db(page_id:str, title:str)->Optional[str]:
...
def sum_numeric_column(db_id: str, col: str) -> float:
    ...
    for row in query_db(db_id):
        ...
        t = cell.get("type")
        if t=="number":  total += float(cell.get("number") or 0)
        elif t=="formula": total += float((cell.get("formula") or {}).get("number") or 0)
        elif t=="rollup":
            r = cell.get("rollup") or {}
            if r.get("type")=="number": total += float(r.get("number") or 0)
    return round(total,2)

# ===== إنشاء/كتابة قاعدة الربحية =====
...
def get_or_create_profit_db()->str:
    if PROFIT_DB_ID_ENV: return hyphenate(PROFIT_DB_ID_ENV)
...
def main():
    ...
    try:
        url = retrieve_db(out_db).get("url")
        print(f"📄 افتح قاعدة الربحية: {url}")
    except Exception:
        pass
    print("🎯 اكتمل.")
...
if __name__ == "__main__":
    try:
        ...
        print("❌ HTTPError:", e)
        try: print("↪️", e.response.status_code, e.response.text)
        except: pass
