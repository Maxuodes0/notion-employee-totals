...
TRANSFERRED_PROJECTS_PROP = "مشاريع محوّلة"
NOT_TRANSFERRED_PROJECTS_PROP = "مشاريع غير محوّلة"
...
STATUS_KEYWORDS = ["status", "transfer", "تحويل", "حالة", "الحوالة", "محول", "محولة", "محوّلة"]
...
assert API_KEY, "🚫 أضف NOTION_API_KEY أو NOTION_TOKEN"
...
def is_transferred(label):
    return normalize_status(label) in TRANSFER_TRUE_VALUES
...
def validate_totals_database():
    ...
    missing = [prop for prop in required if prop not in props]

    if missing:
        print("الأعمدة الموجودة فعليًا في Notion:")
...
def update_totals_database():
    ...
    for employee_name, data in totals.items():
        action = upsert_employee(employee_name, data)
